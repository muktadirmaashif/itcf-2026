...
export const PlayerListView = () => {
  const { players, teams, getTeamById } = useAuction();
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-700/50">
                    ...
                    {catPlayers.map(player => (
                      <tr key={player.id} className="hover:bg-slate-750/50 transition-colors">
                        ...
                        <td className="px-6 py-4 font-medium text-slate-300">
                            {player.soldToTeamId ? getTeamById(player.soldToTeamId)?.name : <span className="text-slate-600 italic">---</span>}
                        </td>
